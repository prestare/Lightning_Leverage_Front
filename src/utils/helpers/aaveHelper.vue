...
import { Contract, Signer, ethers, BigNumber, providers } from "ethers";
import {
    AAVE_POOL_ADDRESS,
...
import { AAVE_POOL, WETH_GATEWAY, AavePriceOricle } from "../../common/GLOBAL.vue";
import { num2Fixed } from "./generalHelper";
...
export const aTokenContract = (aTokenAddress: string, provider: providers.Provider) => {
...
export const debtTokenContract = (debtTokenAddress: string, provider: providers.Provider) => {
    return (new ethers.Contract(debtTokenAddress, debtTokenABI, provider));
}
